...
FString Polymorphic<FString, void>::read(SerializationCtx &ctx,
                                         Buffer &buffer) {
...
void Polymorphic<FString, void>::write(SerializationCtx &ctx, Buffer &buffer,
                                       FString const &value) {
  buffer.write_char16_string(reinterpret_cast<const uint16_t *>(GetData(value)),
                             value.Len());
}
