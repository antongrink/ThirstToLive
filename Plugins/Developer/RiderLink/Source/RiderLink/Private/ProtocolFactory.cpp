...
static FString GetLocalAppdataFolder() {
  ...
  FPlatformMisc::GetEnvironmentVariable(*EnvironmentVarName, CAppDataLocalPath,
                                        ARRAY_COUNT(CAppDataLocalPath));
...
std::shared_ptr<rd::SocketWire::Server>
ProtocolFactory::CreateWire(rd::IScheduler *Scheduler,
                            rd::Lifetime SocketLifetime) {
  const FString ProjectName = GetProjectName();
  return std::make_shared<rd::SocketWire::Server>(
      SocketLifetime, Scheduler, 0,
      TCHAR_TO_UTF8(
          *FString::Printf(TEXT("UnrealEditorServer-%s"), *ProjectName)));
}

TUniquePtr<rd::Protocol>
ProtocolFactory::CreateProtocol(rd::IScheduler *Scheduler,
                                rd::Lifetime SocketLifetime,
                                std::shared_ptr<rd::SocketWire::Server> wire) {
  const FString ProjectName = GetProjectName();

  auto protocol = MakeUnique<rd::Protocol>(rd::Identities::SERVER, Scheduler,
                                           wire, SocketLifetime);
  ...
  if (PlatformFile.CreateDirectoryTree(*PortFullDirectoryPath) &&
    ...
    const FString TmpPortFileFullPath =
        FPaths::Combine(*PortFullDirectoryPath, *TmpPortFile);
    FFileHelper::SaveStringToFile(FString::FromInt(wire->port),
                                  *TmpPortFileFullPath);
    const FString PortFileFullPath =
        FPaths::Combine(*PortFullDirectoryPath, *ProjectName);
    IFileManager::Get().Move(*PortFileFullPath, *TmpPortFileFullPath, true,
                             true);
  }
  return protocol;
}
