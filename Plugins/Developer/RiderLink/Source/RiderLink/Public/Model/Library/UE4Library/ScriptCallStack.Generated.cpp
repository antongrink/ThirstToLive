...
ScriptCallStack::ScriptCallStack(
    ...
    : IScriptCallStack(), frames_(std::move(frames_)) {
  initialize();
}
// secondary constructor
// default ctors and dtors
// reader
ScriptCallStack ScriptCallStack::read(rd::SerializationCtx &ctx,
                                      rd::Buffer &buffer) {
  auto frames_ =
      buffer.read_array<TArray, ScriptCallStackFrame, FDefaultAllocator>(
          [&ctx, &buffer]() mutable {
...
// writer
void ScriptCallStack::write(rd::SerializationCtx &ctx,
                            rd::Buffer &buffer) const {
  buffer.write_array<TArray, ScriptCallStackFrame, FDefaultAllocator>(
      frames_, [&ctx, &buffer](ScriptCallStackFrame const &it) mutable -> void {
...
// identify
// getters
TArray<rd::Wrapper<ScriptCallStackFrame>> const &
ScriptCallStack::get_frames() const {
  return frames_;
}
// intern
// equals trait
...
// type name trait
std::string ScriptCallStack::type_name() const { return "ScriptCallStack"; }
// static type name trait
std::string ScriptCallStack::static_type_name() { return "ScriptCallStack"; }
...
// external to string
std::string to_string(const ScriptCallStack &value) { return value.toString(); }
} // namespace EditorPlugin
} // namespace JetBrains
