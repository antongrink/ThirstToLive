...
// deconstruct trait
  ...
  template <size_t I> decltype(auto) get() const {
    if constexpr (I < 0 || I >= 4)
      static_assert(I < 0 || I >= 4, "I < 0 || I >= 4");
    else if constexpr (I == 0)
      return static_cast<const LogMessageInfo &>(get_info());
    else if constexpr (I == 1)
      return static_cast<const FString &>(get_text());
    else if constexpr (I == 2)
      return static_cast<const TArray<rd::Wrapper<StringRange>> &>(
          get_bpPathRanges());
...
template <> struct hash<JetBrains::EditorPlugin::UnrealLogEvent> {
  size_t operator()(
      const JetBrains::EditorPlugin::UnrealLogEvent &value) const noexcept {
...
template <>
class tuple_size<JetBrains::EditorPlugin::UnrealLogEvent>
...
public:
  using type =
      decltype(declval<JetBrains::EditorPlugin::UnrealLogEvent>().get<I>());
};
