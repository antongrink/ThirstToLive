...
public:
  // primary ctor
  LogMessageInfo(ELogVerbosity::Type type_, FString category_,
                 rd::optional<rd::DateTime> time_);
...
// deconstruct trait
  ...
  template <size_t I> decltype(auto) get() const {
    if constexpr (I < 0 || I >= 3)
      static_assert(I < 0 || I >= 3, "I < 0 || I >= 3");
    else if constexpr (I == 0)
      return static_cast<const ELogVerbosity::Type &>(get_type());
    else if constexpr (I == 1)
      return static_cast<const FString &>(get_category());
    else if constexpr (I == 2)
      return static_cast<const rd::optional<rd::DateTime> &>(get_time());
  }
#endif
...
template <> struct hash<JetBrains::EditorPlugin::LogMessageInfo> {
  size_t operator()(
      const JetBrains::EditorPlugin::LogMessageInfo &value) const noexcept {
...
template <>
class tuple_size<JetBrains::EditorPlugin::LogMessageInfo>
...
public:
  using type =
      decltype(declval<JetBrains::EditorPlugin::LogMessageInfo>().get<I>());
};
