...
// deconstruct trait
  ...
  template <size_t I> decltype(auto) get() const {
    if constexpr (I < 0 || I >= 2)
      ...
      return static_cast<const int32_t &>(get_first());
    else if constexpr (I == 1)
      return static_cast<const int32_t &>(get_last());
  }
#endif
...
template <> struct hash<JetBrains::EditorPlugin::StringRange> {
  size_t
  operator()(const JetBrains::EditorPlugin::StringRange &value) const noexcept {
...
template <>
class tuple_size<JetBrains::EditorPlugin::StringRange>
...
public:
  using type =
      decltype(declval<JetBrains::EditorPlugin::StringRange>().get<I>());
};
