...
// deconstruct trait
  ...
  template <size_t I> decltype(auto) get() const {
    if constexpr (I < 0 || I >= 1)
      static_assert(I < 0 || I >= 1, "I < 0 || I >= 1");
    else if constexpr (I == 0)
      return static_cast<const FString &>(get_pathName());
  }
#endif
...
public:
  // equality operators
  friend bool operator==(const BlueprintReference &lhs,
                         const BlueprintReference &rhs);
  friend bool operator!=(const BlueprintReference &lhs,
                         const BlueprintReference &rhs);
  // hash code trait
...
template <> struct hash<JetBrains::EditorPlugin::BlueprintReference> {
  size_t operator()(
      const JetBrains::EditorPlugin::BlueprintReference &value) const noexcept {
...
template <>
class tuple_size<JetBrains::EditorPlugin::BlueprintReference>
...
public:
  using type =
      decltype(declval<JetBrains::EditorPlugin::BlueprintReference>().get<I>());
};
