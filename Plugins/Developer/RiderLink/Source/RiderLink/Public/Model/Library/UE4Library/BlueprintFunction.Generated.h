...
// deconstruct trait
  ...
  template <size_t I> decltype(auto) get() const {
    if constexpr (I < 0 || I >= 2)
      ...
      return static_cast<const UClass &>(get_class());
    else if constexpr (I == 1)
      return static_cast<const FString &>(get_name());
  }
#endif
...
public:
  // equality operators
  friend bool operator==(const BlueprintFunction &lhs,
                         const BlueprintFunction &rhs);
  friend bool operator!=(const BlueprintFunction &lhs,
                         const BlueprintFunction &rhs);
  // hash code trait
...
template <> struct hash<JetBrains::EditorPlugin::BlueprintFunction> {
  size_t operator()(
      const JetBrains::EditorPlugin::BlueprintFunction &value) const noexcept {
...
template <>
class tuple_size<JetBrains::EditorPlugin::BlueprintFunction>
...
public:
  using type =
      decltype(declval<JetBrains::EditorPlugin::BlueprintFunction>().get<I>());
};
