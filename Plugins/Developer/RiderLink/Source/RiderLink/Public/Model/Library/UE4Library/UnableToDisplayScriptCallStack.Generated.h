...
public:
  // constants
  static constexpr rd::wstring_view message{
      L"Unable to display Script Callstack. Compile with DO_BLUEPRINT_GUARD=1",
      69};
...
public:
  ...
  UnableToDisplayScriptCallStack(UnableToDisplayScriptCallStack const &) =
      default;

  UnableToDisplayScriptCallStack &
  operator=(UnableToDisplayScriptCallStack const &) = default;

  UnableToDisplayScriptCallStack(UnableToDisplayScriptCallStack &&) = default;

  UnableToDisplayScriptCallStack &
  operator=(UnableToDisplayScriptCallStack &&) = default;

  virtual ~UnableToDisplayScriptCallStack() = default;

  // reader
  static UnableToDisplayScriptCallStack read(rd::SerializationCtx &ctx,
                                             rd::Buffer &buffer);
...
public:
  // equality operators
  friend bool operator==(const UnableToDisplayScriptCallStack &lhs,
                         const UnableToDisplayScriptCallStack &rhs);
  friend bool operator!=(const UnableToDisplayScriptCallStack &lhs,
                         const UnableToDisplayScriptCallStack &rhs);
  // hash code trait
...
struct hash<JetBrains::EditorPlugin::UnableToDisplayScriptCallStack> {
  size_t operator()(
      const JetBrains::EditorPlugin::UnableToDisplayScriptCallStack &value)
      const noexcept {
