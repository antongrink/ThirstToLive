...
// primary ctor
LogMessageInfo::LogMessageInfo(ELogVerbosity::Type type_, FString category_,
                               rd::optional<rd::DateTime> time_)
    : rd::IPolymorphicSerializable(), type_(std::move(type_)),
      category_(std::move(category_)), time_(std::move(time_)) {
  initialize();
}
// secondary constructor
// default ctors and dtors
// reader
LogMessageInfo LogMessageInfo::read(rd::SerializationCtx &ctx,
                                    rd::Buffer &buffer) {
  ...
  auto time_ = buffer.read_nullable<rd::DateTime>(
      [&ctx, &buffer]() mutable { return buffer.read_date_time(); });
  LogMessageInfo res{std::move(type_), std::move(category_), std::move(time_)};
  return res;
}
// writer
void LogMessageInfo::write(rd::SerializationCtx &ctx,
                           rd::Buffer &buffer) const {
  rd::Polymorphic<ELogVerbosity::Type>::write(ctx, buffer, type_);
  rd::Polymorphic<std::decay_t<decltype(category_)>>::write(ctx, buffer,
                                                            category_);
  buffer.write_nullable<rd::DateTime>(
      time_, [&ctx, &buffer](rd::DateTime const &it) mutable -> void {
...
// identify
// getters
ELogVerbosity::Type const &LogMessageInfo::get_type() const { return type_; }
FString const &LogMessageInfo::get_category() const { return category_; }
...
size_t LogMessageInfo::hashCode() const noexcept {
  ...
  __r = __r * 31 + ((static_cast<bool>(get_time()))
                        ? rd::hash<rd::DateTime>()(*get_time())
                        : 0);
  return __r;
}
// type name trait
std::string LogMessageInfo::type_name() const { return "LogMessageInfo"; }
// static type name trait
std::string LogMessageInfo::static_type_name() { return "LogMessageInfo"; }
...
// external to string
std::string to_string(const LogMessageInfo &value) { return value.toString(); }
} // namespace EditorPlugin
} // namespace JetBrains
