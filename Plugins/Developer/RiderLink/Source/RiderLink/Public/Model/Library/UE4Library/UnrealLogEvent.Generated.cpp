...
UnrealLogEvent::UnrealLogEvent(rd::Wrapper<LogMessageInfo> info_, FString text_,
    ...
    : rd::IPolymorphicSerializable(), info_(std::move(info_)),
      text_(std::move(text_)), bpPathRanges_(std::move(bpPathRanges_)),
      methodRanges_(std::move(methodRanges_)) {
  initialize();
}
// secondary constructor
// default ctors and dtors
// reader
UnrealLogEvent UnrealLogEvent::read(rd::SerializationCtx &ctx,
                                    rd::Buffer &buffer) {
  auto info_ = LogMessageInfo::read(ctx, buffer);
  auto text_ = rd::Polymorphic<FString>::read(ctx, buffer);
  auto bpPathRanges_ =
      buffer.read_array<TArray, StringRange, FDefaultAllocator>(
          [&ctx, &buffer]() mutable { return StringRange::read(ctx, buffer); });
  auto methodRanges_ =
      buffer.read_array<TArray, StringRange, FDefaultAllocator>(
          [&ctx, &buffer]() mutable { return StringRange::read(ctx, buffer); });
  UnrealLogEvent res{std::move(info_), std::move(text_),
                     std::move(bpPathRanges_), std::move(methodRanges_)};
  return res;
}
// writer
void UnrealLogEvent::write(rd::SerializationCtx &ctx,
  ...
  buffer.write_array<TArray, StringRange, FDefaultAllocator>(
      bpPathRanges_, [&ctx, &buffer](StringRange const &it) mutable -> void {
        rd::Polymorphic<std::decay_t<decltype(it)>>::write(ctx, buffer, it);
      });
  buffer.write_array<TArray, StringRange, FDefaultAllocator>(
      methodRanges_, [&ctx, &buffer](StringRange const &it) mutable -> void {
        rd::Polymorphic<std::decay_t<decltype(it)>>::write(ctx, buffer, it);
      });
}
// virtual init
// identify
// getters
LogMessageInfo const &UnrealLogEvent::get_info() const { return *info_; }
FString const &UnrealLogEvent::get_text() const { return text_; }
TArray<rd::Wrapper<StringRange>> const &
UnrealLogEvent::get_bpPathRanges() const {
  return bpPathRanges_;
}
TArray<rd::Wrapper<StringRange>> const &
UnrealLogEvent::get_methodRanges() const {
...
bool UnrealLogEvent::equals(rd::ISerializable const &object) const {
  ...
  if (this == &other)
    return true;
  if (this->info_ != other.info_)
    return false;
  if (this->text_ != other.text_)
    return false;
  if (this->bpPathRanges_ != other.bpPathRanges_)
    return false;
  if (this->methodRanges_ != other.methodRanges_)
    return false;
...
// type name trait
std::string UnrealLogEvent::type_name() const { return "UnrealLogEvent"; }
// static type name trait
std::string UnrealLogEvent::static_type_name() { return "UnrealLogEvent"; }
...
// external to string
std::string to_string(const UnrealLogEvent &value) { return value.toString(); }
} // namespace EditorPlugin
} // namespace JetBrains
