...
// deconstruct trait
  ...
  template <size_t I> decltype(auto) get() const {
    if constexpr (I < 0 || I >= 2)
      static_assert(I < 0 || I >= 2, "I < 0 || I >= 2");
    else if constexpr (I == 0)
      return static_cast<const int32_t &>(get_begin());
    else if constexpr (I == 1)
      return static_cast<const int32_t &>(get_end());
  }
#endif
  ...
  // reader
  static BlueprintHighlighter read(rd::SerializationCtx &ctx,
                                   rd::Buffer &buffer);
...
public:
  // equality operators
  friend bool operator==(const BlueprintHighlighter &lhs,
                         const BlueprintHighlighter &rhs);
  friend bool operator!=(const BlueprintHighlighter &lhs,
                         const BlueprintHighlighter &rhs);
  // hash code trait
...
template <> struct hash<JetBrains::EditorPlugin::BlueprintHighlighter> {
  size_t operator()(const JetBrains::EditorPlugin::BlueprintHighlighter &value)
      const noexcept {
...
template <>
class tuple_size<JetBrains::EditorPlugin::BlueprintHighlighter>
    : public integral_constant<size_t, 2> {};

template <size_t I>
class tuple_element<I, JetBrains::EditorPlugin::BlueprintHighlighter> {
public:
  using type = decltype(declval<JetBrains::EditorPlugin::BlueprintHighlighter>()
                            .get<I>());
};
