...
inline std::string to_string(FString const &val) { return TCHAR_TO_UTF8(*val); }

template <typename T, typename A> int32_t size(TArray<T, A> const &value) {
...
namespace rd {
template <> class Polymorphic<FString> {
public:
...
template <> class Polymorphic<Wrapper<FString>> {
public:
  static void write(SerializationCtx &ctx, Buffer &buffer,
                    Wrapper<FString> const &value);
};
...
template <typename T> struct hash<TArray<T>> {
  size_t operator()(const TArray<T> &value) const noexcept;
};

template <typename T> Wrapper<T> ToRdWrapper(TUniquePtr<T> &&Ptr) {
  Wrapper<T> Result;
