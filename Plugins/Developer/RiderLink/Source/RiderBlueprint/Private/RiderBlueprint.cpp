...
static void AllowSetForeGroundForEditor(
    JetBrains::EditorPlugin::RdEditorModel const &unrealToBackendModel) {
  static const int32 CurrentProcessId = FPlatformProcess::GetCurrentProcessId();
  try {
    const rd::WiredRdTask<bool> Task =
        unrealToBackendModel.get_allowSetForegroundWindow().sync(
            CurrentProcessId);
    if (Task.is_faulted()) {
      UE_LOG(FLogRiderBlueprintModule, Error,
             TEXT("AllowSetForeGroundForEditor failed: %hs "),
             rd::to_string(Task.value_or_throw()).c_str());
...
void FRiderBlueprintModule::StartupModule() {
  ...
  MessageEndpoint =
      FMessageEndpoint::Builder(FName("FAssetEditorManager")).Build();
  ...
  RiderLinkModule.ViewModel(
      ModuleLifetimeDef.lifetime,
      [this](
          rd::Lifetime ModelLifetime,
          JetBrains::EditorPlugin::RdEditorModel const &UnrealToBackendModel) {
        UnrealToBackendModel.get_openBlueprint().advise(
            ...
            [this, &UnrealToBackendModel](
              ...
              try {
                ...
                BluePrintProvider::OpenBlueprint(s.get_pathName(),
                                                 MessageEndpoint);
              } catch (std::exception const &e) {
        ...
        UnrealToBackendModel.get_isBlueprintPathName().set(
            [](FString const &pathName) -> bool {
