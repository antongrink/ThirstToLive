...
namespace rd {
SequentialLifetimes::SequentialLifetimes(Lifetime parent_lifetime)
    : parent_lifetime(std::move(parent_lifetime)) {
...
Lifetime SequentialLifetimes::next() {
  std::shared_ptr<LifetimeDefinition> new_def =
      std::make_shared<LifetimeDefinition>(parent_lifetime);
...
void SequentialLifetimes::set_current_lifetime(
    std::shared_ptr<LifetimeDefinition> new_def) {
