...
LifetimeImpl::LifetimeImpl(bool is_eternal)
    : eternaled(is_eternal), id(LifetimeImpl::get_id++) {}
...
bool LifetimeImpl::is_terminated() const { return terminated; }

bool LifetimeImpl::is_eternal() const { return eternaled; }
...
LifetimeImpl::~LifetimeImpl() {
  /*if (!is_eternal() && !is_terminated()) {
          spdlog::error("forget to terminate lifetime with id: {}",
  to_string(id)); terminate();
  }*/
}
} // namespace rd
