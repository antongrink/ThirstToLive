...
namespace rd {
namespace detail {
template <typename T> class ListEvent {
public:
  class Add {
  public:
    int32_t index;
    ...
    Add(int32_t index, T const *new_value)
        : index(index), new_value(new_value) {}
  };
  ...
  public:
    ...
    Remove(int32_t index, T const *old_value)
        : index(index), old_value(old_value) {}
  };
  ...
  int32_t get_index() const {
    return visit(util::make_visitor([](Add const &e) { return e.index; },
                                    [](Update const &e) { return e.index; },
                                    [](Remove const &e) { return e.index; }),
                 v);
  }

  T const *get_new_value() const {
    return visit(util::make_visitor([](Add const &e) { return e.new_value; },
                                    [](Update const &e) { return e.new_value; },
                                    [](Remove const & /*e*/) {
    ...
    std::string res = visit(
        util::make_visitor(
            [](typename ListEvent::Add const &e) {
              return "Add " + std::to_string(e.index) + ":" +
...
protected:
  mutable rd::unordered_map<Lifetime, std::vector<LifetimeDefinition>>
      lifetimes;
...
public:
  ...
  /**
   * \brief Adds a subscription to additions and removals of list elements. When
   * a list element is updated, the [handler] is called twice: to report the
   ...
   */
  void advise_add_remove(
      Lifetime lifetime,
      std::function<void(AddRemove, size_t, T const &)> handler) const {
   ...
   */
  void view(Lifetime lifetime,
            std::function<void(Lifetime, size_t, T const &)> handler) const {
    advise_add_remove(
        lifetime,
        [this, lifetime, handler](AddRemove kind, size_t idx, T const &value) {
          switch (kind) {
          case AddRemove::ADD: {
  ...
  template <typename... Args> bool emplace_add(Args &&...args) const {
  ...
  template <typename U>
  friend typename std::enable_if<(!std::is_abstract<U>::value),
                                 std::vector<U>>::type
