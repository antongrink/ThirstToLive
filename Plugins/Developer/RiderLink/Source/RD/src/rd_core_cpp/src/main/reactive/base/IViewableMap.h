...
namespace rd {
namespace detail {
template <typename K, typename V> class MapEvent {
public:
  class Add {
  public:
    K const *key;
  ...
  public:
    ...
    Update(K const *key, V const *old_value, V const *new_value)
  ...
  K const *get_key() const {
    return visit(util::make_visitor(
                     [](typename MapEvent::Add const &e) { return e.key; },
                     [](typename MapEvent::Update const &e) { return e.key; },
                     [](typename MapEvent::Remove const &e) { return e.key; }),
                 v);
  }
  ...
  V const *get_new_value() const {
    return visit(
        util::make_visitor(
            [](typename MapEvent::Add const &e) { return e.new_value; },
    ...
    std::string res = visit(
        util::make_visitor(
            [](typename MapEvent::Add const &e) -> std::string {
              return "Add " + to_string(*e.key) + ":" + to_string(*e.new_value);
            },
...
class IViewableMap : public IViewable<std::pair<K const *, V const *>>,
...
protected:
  ...
  mutable rd::unordered_map<
      Lifetime,
      ordered_map<K const *, LifetimeDefinition, wrapper::TransparentHash<K>,
...
public:
  ...
  void view(Lifetime lifetime,
    ...
    advise_add_remove(lifetime, [this, lifetime, handler](AddRemove kind,
                                                          K const &key,
                                                          V const &value) {
      const std::pair<K const *, V const *> entry =
          std::make_pair(&key, &value);
      switch (kind) {
      case AddRemove::ADD: {
        if (lifetimes[lifetime].count(key) == 0) {
          /*auto const &[it, inserted] = lifetimes[lifetime].emplace(key,
           * LifetimeDefinition(lifetime));*/
          auto const &pair =
              lifetimes[lifetime].emplace(&key, LifetimeDefinition(lifetime));
          auto &it = pair.first;
      ...
      case AddRemove::REMOVE: {
        RD_ASSERT_MSG(lifetimes.at(lifetime).count(key) > 0,
                      "attempting to remove non-existing lifetime in viewable "
                      "map by key:" +
                          to_string(key));
  ...
  /**
   * \brief Adds a subscription to additions and removals of map elements. When
   * a map element is updated, the [handler] is called twice: to report the
   * removal of the old element and the addition of the new one. \param lifetime
   * lifetime of subscription. \param handler to be called.
   */
  void advise_add_remove(Lifetime lifetime,
                         std::function<void(AddRemove, K const &, V const &)
  ...
  /**
   * \brief Adds a subscription to changes of the contents of the map, with the
   * handler receiving keys and values as separate parameters.
   *
   * \details When [handler] is initially added, it is called receiving all keys
   * and values currently in the map. Every time a key/value pair is added to
   * the map, the [handler] is called receiving the new key and value. The
   * [Lifetime] instance passed to the handler expires when the key/value pair
   ...
   */
  void view(Lifetime lifetime,
            std::function<void(Lifetime, K const &, V const &)

                          >
                handler) const {
    view(lifetime,
         [handler](Lifetime lf, const std::pair<K const *, V const *> entry) {
           handler(lf, *entry.first, *entry.second);
         });
  }

  void advise(Lifetime lifetime,
              std::function<void(Event const &)> handler) const override = 0;
...
static_assert(
    std::is_move_constructible<rd::IViewableMap<int, int>::Event>::value,
