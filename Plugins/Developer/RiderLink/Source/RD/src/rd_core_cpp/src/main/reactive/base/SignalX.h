 ...
 */
template <typename T> class Signal final : public ISignal<T> {
private:
  ...
  public:
    ...
    template <typename F>
    Event(F &&action, Lifetime lifetime)
        : action(std::forward<F>(action)), lifetime(lifetime) {}

    Event(Event &&) = default;
    // endregion

    bool is_alive() const { return !lifetime->is_terminated(); }
  ...
  template <typename F>
  void advise0(const Lifetime &lifetime, F &&handler,
               listeners_t &queue) const {
...
public:
  ...
  void advise(Lifetime lifetime,
              std::function<void(T const &)> handler) const override {
    advise0(lifetime, std::move(handler),
            isPriorityAdvise() ? priority_listeners : listeners);
  }

  static bool isPriorityAdvise() { return rd_signal_cookie_get() > 0; }
};

template <typename F> void priorityAdviseSection(F &&block) {
...
static_assert(std::is_move_constructible<rd::Signal<int>>::value,
              "Is not move constructible from Signal<int>");
static_assert(std::is_move_constructible<rd::Signal<rd::Void>>::value,
              "Is not move constructible from Signal<Void>");
