...
protected:
  ...
  const std::vector<Wrapper<T>> &getList() const override { return list; }
...
public:
  ...
  public:
    ...
    pointer operator->() noexcept { return (*it_).get(); }
  ...
  bool remove(T const &element) const override {
    auto it = std::find_if(list.begin(), list.end(),
                           [&element](auto const &p) { return *p == element; });
    if (it == list.end()) {
      return false;
    }
  ...
  T const &get(size_t index) const override { return *list[index]; }
  ...
  void clear() const override {
    ...
    for (size_t i = size(); i > 0; --i) {
      changes.push_back(
          typename Event::Remove(static_cast<int32_t>(i - 1), &(*list[i - 1])));
    }
