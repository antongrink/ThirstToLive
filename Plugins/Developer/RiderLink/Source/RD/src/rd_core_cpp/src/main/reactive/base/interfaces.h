 ...
 */
template <typename T> class ISource {
public:
  ...
  virtual void advise(Lifetime lifetime,
                      std::function<void(T const &)> handler) const = 0;

  /**
   * \brief @code advise with Eternal lifetime
   */
  template <typename F> void advise_eternal(F &&handler) const {
 ...
 */
template <typename T> class IViewable {
public:
...
/**
 * \brief An object which has a collection of event listeners and can broadcast
 * an event to the listeners. \tparam T type of events
 */
template <typename T> class ISignal : public ISource<T> {
protected:
