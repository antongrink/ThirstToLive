...
namespace rd {
namespace detail {
template <typename T> class SetEvent {
public:
...
protected:
  ...
  mutable rd::unordered_map<
      Lifetime,
      ordered_map<T const *, LifetimeDefinition, wrapper::TransparentHash<T>,
                  wrapper::TransparentKeyEqual<T>>>
      lifetimes;
...
public:
  ...
  /**
   * \brief Adds a subscription for additions and removals of set elements. When
   * the subscription is initially added, [handler] is called with
   * [AddRemove::Add] events for all elements currently in the set.
   *
   * \param lifetime lifetime of subscription.
   * \param handler to be called.
   */
  void advise(Lifetime lifetime,
              std::function<void(AddRemove, T const &)> handler) const {
   ...
   *
   * \details When [handler] is initially added, it is called receiving all
   * elements currently in the set. Every time an object is added to the set,
   * the [handler] is called receiving the new element. The [Lifetime] instance
   * passed to the handler expires when the element is removed from the set.
   *
   * \param lifetime
   * \param handler
   */
  void view(Lifetime lifetime,
            std::function<void(Lifetime, T const &)> handler) const override {
    advise(lifetime, [this, lifetime, handler](AddRemove kind, T const &key) {
      ...
      case AddRemove::ADD: {
        ...
        RD_ASSERT_MSG(
            it.second,
            "lifetime definition already exists in viewable set by key:" +
                to_string(key));
