 ...
 */
template <typename T> class IProperty : public IPropertyBase<T> {
protected:
...
private:
  void advise0(Lifetime lifetime, std::function<void(T const &)> handler,
               Signal<T> const &signal) const {
  ...
  void advise_before(Lifetime lifetime,
...
public:
   ...
   */
  template <typename... Args> void emplace(Args &&...args) const {
