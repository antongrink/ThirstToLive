 ...
 */
template <typename K, typename V, typename KA = std::allocator<K>,
...
private:
  ...
  using PA = typename std::allocator_traits<VA>::template rebind_alloc<
      std::pair<Wrapper<K>, Wrapper<V>>>;

  Signal<Event> change;

  using data_t =
      ordered_map<Wrapper<K>, Wrapper<V>, wrapper::TransparentHash<K>,
...
public:
  ...
  public:
    ...
    reference operator*() const noexcept { return *it_.value(); }

    pointer operator->() const noexcept { return it_.value().get(); }

    key_type const &key() const { return *it_.key(); }

    value_type const &value() const { return *it_.value(); }
  };
  ...
  const V *set(WK key, WV value) const override {
    if (map.count(key) == 0) {
      /*auto[it, success] = map.emplace(std::make_unique<K>(std::move(key)),
       * std::make_unique<V>(std::move(value)));*/
      ...
      if (*value_ptr != wrapper::get<V>(value)) { // TO-DO more effective
