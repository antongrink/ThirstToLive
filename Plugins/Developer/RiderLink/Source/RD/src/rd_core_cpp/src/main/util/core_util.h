...
namespace wrapper {
template <typename T> struct TransparentKeyEqual {
...
template <typename T> struct TransparentHash {
  using is_transparent = void;
  using transparent_key_equal = std::equal_to<>;

  size_t operator()(T const &val) const noexcept { return rd::hash<T>()(val); }
