...
public:
  ...
  SerializationCtx
  withInternRootsHere(RdBindableBase const &owner,
                      std::initializer_list<std::string> new_roots) const;

  // endregion

  template <typename T, util::hash_t InternKey>
  Wrapper<T> readInterned(
      Buffer &buffer,
      std::function<T(SerializationCtx &, Buffer &)> readValueDelegate);

  template <typename T, util::hash_t InternKey, typename F,
            typename = typename std::enable_if_t<
                util::is_invocable<F, SerializationCtx &, Buffer &, T>::value>>
  void writeInterned(Buffer &buffer, Wrapper<T> const &value,
...
namespace rd {
template <typename T, util::hash_t InternKey>
Wrapper<T> SerializationCtx::readInterned(
    Buffer &buffer,
    std::function<T(SerializationCtx &, Buffer &)> readValueDelegate) {
