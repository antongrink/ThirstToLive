...
void Serializers::real_write(SerializationCtx & /*ctx*/, Buffer &buffer,
...
void Serializers::register_in() {
  readers[STRING_PREDEFINED_ID] = [](SerializationCtx &ctx,
                                     Buffer &buffer) -> InternedAny {
    return {wrapper::make_wrapper<std::wstring>(
        Polymorphic<std::wstring>::read(ctx, buffer))};
  };
}

Serializers::Serializers() { register_in(); }
} // namespace rd
