 ...
 * remote copy over a network or a similar connection, and which allows to
 * subscribe to its changes.
 */
class RD_FRAMEWORK_API IRdReactive : public virtual IRdBindable {
public:
  /**
   * \brief If set to true, local changes to this object can be performed on any
   * thread. Otherwise, local changes can be performed only on the UI thread.
   */
  bool async = false;
  // region ctor/dtor
  ...
  /**
   * \brief Scheduler on which wire invokes callback [onWireReceived]. Default
   * is the same as [protocol]'s one. \return scheduler
   */
