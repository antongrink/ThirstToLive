...
std::shared_ptr<spdlog::logger> SocketWire::Base::logger =
    spdlog::stderr_color_mt<spdlog::synchronous_factory>(
        "wireLog", spdlog::color_mode::automatic);
...
SocketWire::Base::Base(std::string id, Lifetime parentLifetime,
                       IScheduler *scheduler)
    : WireBase(scheduler), id(std::move(id)), scheduler(scheduler),
      lifetimeDef(parentLifetime) {
...
bool SocketWire::Base::send0(Buffer::ByteArray const &msg,
                             sequence_number_t seqn) const {
  try {
    ...
    RD_ASSERT_THROW_MSG(
        socket_provider->Send(send_package_header.data(),
                              send_package_header.get_position()) ==
...
void SocketWire::Base::send(RdId const &rd_id,
                            std::function<void(Buffer &buffer)> writer) const {
...
void SocketWire::Base::set_socket_provider(
    std::shared_ptr<CActiveSocket> new_socket) {
  {
    std::lock_guard<decltype(socket_send_lock)> guard(socket_send_lock);
  ...
  logger->debug("{}: waited for heartbeat to stop with status: {}", this->id,
...
bool SocketWire::Base::connection_established(int32_t timestamp,
                                              int32_t notion_timestamp) {
...
bool SocketWire::Base::read_from_socket(Buffer::word_t *res,
  ...
  while (ptr < msglen) {
    ...
    if (available > 0) {
      ...
      int32_t read = socket_provider->Receive(
          static_cast<int32_t>(receiver_buffer.end() - hi), &*hi);
      if (read == -1) {
  ...
  if (ptr != msglen) {
    logger->error(
        "read invalid number of bytes from socket, expected: {}, actual: {}",
        msglen, ptr);
    assert(false);
  }
  return true;
...
static constexpr std::pair<int, sequence_number_t> INVALID_HEADER =
    std::make_pair(-1, -1);
...
std::pair<int, sequence_number_t> SocketWire::Base::read_header() const {
  ...
  while (true) {
    ...
    if (len == PING_MESSAGE_LENGTH) {
      ...
      if ((connection_established(current_timestamp,
                                  counterpart_acknowledge_timestamp))) {
        if (!heartbeatAlive.get()) { // only on change
          logger->trace("Connection is alive after receiving PING {}: "
                        "received_timestamp: {}, "
                        "received_counterpart_timestamp: {}, "
                        "current_timestamp: {}, "
                        "counterpart_timestamp: {}, "
                        "counterpart_acknowledge_timestamp: {}, ",
                        id, received_timestamp, received_counterpart_timestamp,
                        current_timestamp, counterpart_timestamp,
                        counterpart_acknowledge_timestamp);
        }
...
int32_t SocketWire::Base::read_package() const {
  ...
  logger->debug("{}: read len={}, seqn={}, max_received_seqn={}", this->id, len,
                seqn, max_received_seqn);
  ...
  logger->info("{}: was received package, bytes={}, seqn={}", this->id, len,
               seqn);
  return len;
}
...
bool SocketWire::Base::read_and_dispatch_message() const {
  ...
  if (!receive_pkg.read(message.data() + message.get_position(),
                        sz - message.get_position())) {
  ...
  message.rewind();
  return true;
  //		RD_ASSERT_MSG(summary_size == sz, "Broken message, read:%d bytes,
  //expected:%d bytes", summary_size, sz)
}
...
void SocketWire::Base::ping() const {
  if (!connection_established(current_timestamp,
                              counterpart_acknowledge_timestamp)) {
    if (heartbeatAlive.get()) { // only on change
      logger->trace("Disconnect detected while sending PING {}: "
                    "current_timestamp: {}, "
                    "counterpart_timestamp: {}, "
                    "counterpart_acknowledge_timestamp: {}",
                    this->id, current_timestamp, counterpart_timestamp,
                    counterpart_acknowledge_timestamp);
    }
  ...
  try {
    ...
    {
      std::lock_guard<decltype(socket_send_lock)> guard(socket_send_lock);
      int32_t sent = socket_provider->Send(ping_pkg_header.data(),
                                           ping_pkg_header.get_position());
      if (sent == 0 && !socket_provider->IsSocketValid()) {
        logger->debug("{}: failed to send ping over the network, reason: "
                      "socket was shut down for sending",
                      this->id);
        return;
      }
      RD_ASSERT_THROW_MSG(
          sent == PACKAGE_HEADER_LENGTH,
          fmt::format("{}: failed to send ping over the network, reason: {}",
                      this->id, socket_provider->DescribeError()))
    }
...
bool SocketWire::Base::send_ack(sequence_number_t seqn) const {
  ...
  try {
    ...
    ack_buffer.write_integral(seqn);
    {
      std::lock_guard<decltype(socket_send_lock)> guard(socket_send_lock);
      RD_ASSERT_THROW_MSG(
          socket_provider->Send(ack_buffer.data(), ack_buffer.get_position()) ==
              PACKAGE_HEADER_LENGTH,
          this->id +
    ...
    logger->warn("{}: exception raised during ACK, seqn = {} | {}", id, seqn,
                 e.what());
    return false;
  }
}
...
SocketWire::Client::Client(Lifetime parentLifetime, IScheduler *scheduler,
                           uint16_t port, const std::string &id)
    : Base(id, parentLifetime, scheduler), port(port),
      clientLifetimeDefinition(parentLifetime) {
  Lifetime lifetime = clientLifetimeDefinition.lifetime;
  thread = std::thread([this, lifetime]() mutable {
    rd::util::set_thread_name(this->id.empty() ? "SocketWire::Client Thread"
                                               : this->id.c_str());
    ...
    try {
      ...
      while (!lifetime->is_terminated()) {
        try {
          ...
          RD_ASSERT_THROW_MSG(
              socket->Initialize(),
              fmt::format("{}: failed to init ActiveSocket, reason: {}",
                          this->id, socket->DescribeError()));
          RD_ASSERT_THROW_MSG(
              socket->DisableNagleAlgoritm(),
              fmt::format("{}: failed to DisableNagleAlgoritm, reason: {}",
                          this->id, socket->DescribeError()));

          // On windows connect will try to send SYN 3 times with interval of
          // 500ms (total time is 1second) Connect timeout doesn't work if it's
          // more than 1 second. But we don't need it because we can close
          // socket any moment.
          ...
          RD_ASSERT_THROW_MSG(
              socket->Open("127.0.0.1", this->port),
              fmt::format("{}: failed to open ActiveSocket, reason: {}",
                          this->id, socket->DescribeError()));
          {
            std::lock_guard<decltype(lock)> guard(lock);
            if (lifetime->is_terminated()) {
              if (!socket->Close()) {
                logger->error("{} failed to close socket, reason: {}", this->id,
          ...
          logger->debug("{}: connection error for port {} ({}).", this->id,
                        this->port, e.what());
  ...
  lifetime->add_action([this]() {
    ...
    logger->debug("{}: send buffer stopped, success: {}", this->id,
                  send_buffer_stopped);
...
SocketWire::Server::Server(Lifetime parentLifetime, IScheduler *scheduler,
                           uint16_t port, const std::string &id)
    : Base(id, parentLifetime, scheduler),
      ss(std::make_unique<CPassiveSocket>()),
      serverLifetimeDefinition(parentLifetime) {
#ifdef SIGPIPE
  signal(SIGPIPE, SIG_IGN);
#endif
  RD_ASSERT_MSG(ss->Initialize(),
                fmt::format("{}: failed to initialize socket, reason: {}",
                            this->id, socket->DescribeError()));
  RD_ASSERT_MSG(
      ss->Listen("127.0.0.1", port),
      fmt::format("{}: failed to listen socket on port: {}, reason: {}",
                  this->id, std::to_string(port), ss->DescribeError()));

  this->port = ss->GetServerPort();
  RD_ASSERT_MSG(this->port != 0,
                fmt::format("{}: port wasn't chosen", this->id));

  logger->info("{}: listening 127.0.0.1/{}", this->id, this->port);
  Lifetime lifetime = serverLifetimeDefinition.lifetime;

  thread = std::thread([this, lifetime]() mutable {
    rd::util::set_thread_name(this->id.empty() ? "SocketWire::Server Thread"
                                               : this->id.c_str());
    ...
    try {
      while (!lifetime->is_terminated()) {
        try {
          ...
          // winsock blocking accept hangs after creating new process with
          // createprocess with inheritHandles=true property. Unreal Engine uses
          // the same logic for handling sockets where they wait for timeout on
          // select before trying to accept connection.
          while (ss->IsSocketValid() && !ss->Select(0, 300)) {
          }

          CActiveSocket *accepted = ss->Accept();
          RD_ASSERT_THROW_MSG(accepted != nullptr,
                              fmt::format("{}: accepting failed, reason: {}",
                                          this->id, ss->DescribeError()));
          socket.reset(accepted);
          logger->info("{}: accepted passive socket {}/{}", this->id,
                       socket->GetClientAddr(), socket->GetClientPort());
          RD_ASSERT_THROW_MSG(socket->DisableNagleAlgoritm(),
                              fmt::format("{}: tcpNoDelay failed, reason: {}",
                                          this->id, socket->DescribeError()));
