...
namespace rd {
class RD_FRAMEWORK_API RdBindableBase
    : public virtual IRdBindable /*, IPrintable*/
{
protected:
...
public:
  ...
  // mutable std::map<std::string, std::any> non_bindable_extensions;//TO-DO
  // concurrency
  ...
  auto getOrCreateExtension(std::string name, Args &&...args) const ->
    ...
    std::shared_ptr<T> new_extension =
...
// T : RdBindableBase
template <typename T> T &withId(T &that, RdId const &id) {
...
template <typename T> T &statics(T &that, int64_t id) {
  RD_ASSERT_MSG(
      (id > 0 && id < RdId::MAX_STATIC_ID),
      ("Expected id > 0 && id < RdId.MaxStaticId, got " + std::to_string(id)));
  return withId(that, RdId(static_cast<int64_t>(id)));
}

template <typename T> T &withIdFromName(T &that, std::string const &name) {
