...
PumpScheduler::PumpScheduler()
    : created_thread_id(std::this_thread::get_id()) {}
...
bool PumpScheduler::is_active() const { return true; }

void PumpScheduler::assert_thread() const {
  RD_ASSERT_MSG(created_thread_id == std::this_thread::get_id(),
                "Illegal thread for current action, must be: " +
                    to_string(created_thread_id) + ", current thread: " +
