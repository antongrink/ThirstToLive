...
void RdExtBase::init(Lifetime lifetime) const {
  ...
  lifetime->bracket(
      [&] {
        extProtocol = std::make_shared<Protocol>(
            parentProtocol->identity, sc,
  ...
  // it's critical to advise before 'Ready' is sent because we advise on
  // SynchronousScheduler

  lifetime->bracket(
      [this, parentWire] { sendState(*parentWire, ExtState::Ready); },
...
void RdExtBase::on_wire_received(Buffer buffer) const {
  ...
  if (serializationHash != counterpartSerializationHash) {
    RD_ASSERT_MSG(false, "serializationHash of ext " + to_string(location) +
                             " doesn't match to counterpart: maybe you forgot "
                             "to generate models?\n"
                             "our: " +
                             to_string(serializationHash) + ", counterpart:" +
                             to_string(counterpartSerializationHash))
  }
}
...
void RdExtBase::traceMe(std::shared_ptr<spdlog::logger> logger,
                        string_view message) const {
  logger->trace("ext {} {}:: {}", to_string(location), to_string(rdid),
                std::string(message));
}
