...
class RdList final : public RdReactiveBase,
                     public ViewableList<T, A>,
...
private:
  ...
  std::string logmsg(Op op, int64_t version, int32_t key,
                     T const *value = nullptr) const {
    return "list " + to_string(location) + " " + to_string(rdid) +
           ":: " + to_string(op) + ":: key = " + std::to_string(key) +
...
public:
      ...
      advise(lifetime, [this, lifetime](typename IViewableList<T>::Event e) {
        ...
        get_wire()->send(rdid, [this, e](Buffer &buffer) {
          ...
          buffer.write_integral<int64_t>(
              ...
              (next_version++ << versionedFlagShift));
          buffer.write_integral<int32_t>(
              static_cast<const int32_t>(e.get_index()));
          ...
          spdlog::get("logSend")->trace(
              logmsg(op, next_version - 1, e.get_index(), new_value));
        });
      });
    });
  ...
  void on_wire_received(Buffer buffer) const override {
    ...
    RD_ASSERT_MSG(version == next_version,
                  ("Version conflict for " + to_string(location) +
                   "}. Expected version " + std::to_string(next_version) +
                   ", received " + std::to_string(version) +
    ...
    case Op::ADD: {
      ...
      spdlog::get("logReceived")
          ->trace(logmsg(op, version, index, &(wrapper::get<T>(value))));

      (index < 0) ? list::add(std::move(value))
                  : list::add(static_cast<size_t>(index), std::move(value));
      break;
    }
    case Op::UPDATE: {
      auto value = S::read(this->get_serialization_context(), buffer);
  ...
  bool add(WT element) const override {
    return local_change([this, element = std::move(element)]() mutable {
      return list::add(std::move(element));
    });
  }

  bool add(size_t index, WT element) const override {
    return local_change([this, index, element = std::move(element)]() mutable {
  ...
  bool addAll(size_t index, std::vector<WT> elements) const override {
    return local_change(
        [&] { return list::addAll(index, std::move(elements)); });
  }
...
static_assert(std::is_move_constructible<rd::RdList<int>>::value,
              "Is move constructible RdList<int>");
