...
namespace rd {
/**
 * \brief Advanced monad result. It is in of following states: Success,
 * Cancelled, Fault; Success -  Execution completed. Result stores in it.
...
public:
  ...
  public:
    ...
    Fault(std::wstring reason_type_fqn, std::wstring reason_message,
          std::wstring reason_as_text)
  ...
  template <typename F> RdTaskResult(F &&v) : v(std::forward<F>(v)) {}
  ...
  static RdTaskResult<T, S> read(SerializationCtx &ctx, Buffer &buffer) {
    ...
    case 2: {
      ...
      return Fault(std::move(reason_type_fqn), std::move(reason_message),
                   std::move(reason_as_text));
    }
    default:
      throw std::invalid_argument("Fail on RdTaskResult reading with kind: " +
                                  std::to_string(kind));
    }
  }
  ...
  bool is_succeeded() const { return v.index() == 0; }

  bool is_canceled() const { return v.index() == 1; }

  void as_canceled(std::function<void(Cancelled const &)> f) const {
    f(rd::get<Cancelled>(v));
  }

  bool is_faulted() const { return v.index() == 2; }
