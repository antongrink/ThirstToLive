...
namespace rd {
/**
 * \brief Simple data buffer. Allows to "SerDes" plenty of types, such as
 * integrals, arrays, etc.
 */
...
private:
  template <int> friend std::wstring read_wstring_spec(Buffer &);

  template <int> friend void write_wstring_spec(Buffer &, wstring_view);
...
public:
  ...
  C<T, A> read_array() {
    ...
    RD_ASSERT_MSG(len >= 0,
                  "read null array(length = " + std::to_string(len) + ")");
    C<T, A> result;
    using rd::resize;
    resize(result, len);
    if (len > 0) {
  ...
  C<value_or_wrapper<T>, A>
  read_array(std::function<value_or_wrapper<T>()> reader) {
    int32_t len = read_integral<int32_t>();
  ...
  void write_array(C<T, A> const &container,
                   std::function<void(T const &)> writer) {
    using rd::size;
  ...
  template <template <class, class> class C, typename T,
            typename A = allocator<Wrapper<T>>>
  void write_array(C<Wrapper<T>, A> const &container,
                   std::function<void(T const &)> writer) {
    using rd::size;
    write_integral<int32_t>(size(container));
    for (auto const &e : container) {
  ...
  template <typename T,
            typename = typename std::enable_if_t<util::is_enum_v<T>>>
  T read_enum() {
  ...
  template <typename T, typename F,
            typename = typename std::enable_if_t<
                util::is_same_v<typename util::result_of_t<F()>, T>>>
  ...
  template <typename T, typename F,
            typename = typename std::enable_if_t<
                util::is_same_v<typename util::result_of_t<F()>, Wrapper<T>>>>
  ...
  write_nullable(optional<T> const &value,
                 std::function<void(T const &)> writer) {
    if (!value) {
      write_bool(false);
    } else {
