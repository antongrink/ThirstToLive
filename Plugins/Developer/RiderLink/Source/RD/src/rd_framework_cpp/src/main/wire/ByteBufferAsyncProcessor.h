...
private:
  ...
  std::function<bool(Buffer::ByteArray const &, sequence_number_t seqn)>
      processor;
...
public:
  ...
  explicit ByteBufferAsyncProcessor(
      std::string id,
      std::function<bool(Buffer::ByteArray const &, sequence_number_t)>
          processor);
