...
namespace rd {
/**
 * \brief A non-root node in an object graph which can be synchronized with its
 * remote copy over a network or a similar connection.
 */
...
public:
  ...
  /**
   * \brief Inserts the node into the object graph under the given [parent] and
   * assigns the specified [name] to it. The node will be removed from the graph
   * when the specified [lf] lifetime is terminated.
   *
   * \param lf lifetime of node.
   * \param parent to whom bind.
   * \param name specified name of node.
   */
  virtual void bind(Lifetime lf, IRdDynamic const *parent,
...
template <typename T>
typename std::enable_if_t<!util::is_base_of_v<
    IRdBindable,
    typename std::decay_t<T>>> inline identifyPolymorphic(T &&,
                                                          Identities const
                                                              & /*identities*/,
                                                          RdId const & /*id*/) {
}
...
inline void identifyPolymorphic(const IRdBindable &that,
                                Identities const &identities, RdId const &id) {
  that.identify(identities, id);
}

template <typename T>
typename std::enable_if_t<util::is_base_of_v<
    IRdBindable, T>> inline identifyPolymorphic(std::vector<T> const &that,
...
typename std::enable_if_t<!util::is_base_of_v<
    IRdBindable,
    typename std::decay_t<T>>> inline bindPolymorphic(T &&, Lifetime /*lf*/,
                                                      const IRdDynamic
                                                          * /*parent*/,
                                                      string_view /*name*/) {}

inline void bindPolymorphic(IRdBindable const &that, Lifetime lf,
...
template <typename T>
typename std::
    enable_if_t<util::is_base_of_v<IRdBindable, T>> inline bindPolymorphic(
        std::vector<T> const &that, Lifetime lf, IRdDynamic const *parent,
        string_view name) {
  for (auto &obj : that) {
