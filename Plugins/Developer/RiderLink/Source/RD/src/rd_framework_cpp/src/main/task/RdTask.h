 ...
 */
template <typename T, typename S = Polymorphic<T>> class RdTask {
protected:
  using WT = value_or_wrapper<T>;

  using TRes = RdTaskResult<T, S>;

  mutable std::shared_ptr<detail::RdTaskImpl<T, S>> impl{
      std::make_shared<detail::RdTaskImpl<T, S>>()};
...
public:
  ...
  template <typename, typename, typename, typename> friend class RdEndpoint;
  // region ctor/dtor
  ...
  void set_result(TRes value) const { impl->result.set(std::move(value)); }

  void set_result_if_empty(TRes value) const {
    impl->result.set_if_empty(std::move(value));
  }

  void cancel() const { impl->result.set(typename TRes::Cancelled()); }

  void fault(std::exception const &e) const {
    impl->result.set(typename TRes::Fault(e));
  }

  bool has_value() const { return impl->result.has_value(); }
  ...
  void advise(Lifetime lifetime,
              std::function<void(TRes const &)> handler) const {
    impl->result.advise(lifetime, [handler = std::move(handler)](
                                      optional<TRes> const &opt_value) {
      if (opt_value) {
        handler(*opt_value);
      }
    });
