...
/**
 * \brief Provides \ref write for serialization to be overriden. For
 * deserialization derived class must have static method read. See examples for
 * more information.
 */
...
public:
   ...
   */
  virtual std::string
  type_name() const = 0 /*{ throw std::invalid_argument("type doesn't support
                           polymorphic serialization"); }*/
      ;

  //		virtual bool equals(IPolymorphicSerializable const& object) const =
  //0;
  ...
  friend bool RD_FRAMEWORK_API operator==(const IPolymorphicSerializable &lhs,
...
namespace std {
template <> struct RD_FRAMEWORK_API hash<rd::IPolymorphicSerializable> {
