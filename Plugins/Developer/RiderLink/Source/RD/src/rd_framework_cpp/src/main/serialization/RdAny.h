...
/**
 * \brief Presents union type to be interned. It may be either \link
 * std::wstring, either \link rd::IPolymorphicSerializable.
 */
using InternedAny = variant<any::wrapped_super_t, any::string>;

namespace any {
template <typename T> InternedAny make_interned_any(Wrapper<T> wrapper) {
  return {wrapped_super_t(wrapper)};
}

template <>
inline InternedAny
make_interned_any<std::wstring>(Wrapper<std::wstring> wrapper) {
  return {wrapper};
}

template <typename T, typename Any>
typename std::enable_if_t<!util::is_base_of_v<IPolymorphicSerializable, T>,
                          any::string>
get(Any const &any) {
...
struct TransparentKeyEqual {
  ...
  bool operator()(InternedAny const &val_l,
                  wrapped_super_t const &val_r) const {
    return visit(
        util::make_visitor(
            [&](wrapped_super_t const &value) { return *value == *val_r; },
            [](any::string const &) { return false; }),
        val_l);
  }
  ...
  bool operator()(wrapped_super_t const &val_l,
                  InternedAny const &val_r) const {
    return operator()(val_r, val_l);
  }

  bool operator()(InternedAny const &val_l, any::string const &val_r) const {
    return visit(util::make_visitor(
                     [](wrapped_super_t const & /*value*/) { return false; },
                     [&](any::string const &s) { return s == val_r; }),
                 val_l);
  }
...
static_assert(!std::is_trivially_copy_constructible<
                  rd::Wrapper<rd::IPolymorphicSerializable>>::value,
