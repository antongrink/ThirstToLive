...
private:
  ...
  // template<typename T>
  mutable ordered_map<InternedAny, int32_t, any::TransparentHash,
                      any::TransparentKeyEqual>
      inverse_map;
...
public:
  ...
  template <typename T> int32_t intern_value(Wrapper<T> value) const;

  template <typename T> Wrapper<T> un_intern_value(int32_t id) const;

  IScheduler *get_wire_scheduler() const override;

  void bind(Lifetime lf, IRdDynamic const *parent,
...
template <typename T> Wrapper<T> InternRoot::un_intern_value(int32_t id) const {
  // don't need lock because value's already exists and never removes
  return any::get<T>(is_index_owned(id) ? my_items_lis[id / 2]
                                        : other_items_list[id / 2]);
}

template <typename T> int32_t InternRoot::intern_value(Wrapper<T> value) const {
  ...
  if (it == inverse_map.end()) {
    get_protocol()->get_wire()->send(
        this->rdid, [this, &index, value, any](Buffer &buffer) {
          InternedAnySerializer::write<T>(get_serialization_context(), buffer,
                                          wrapper::get<T>(value));
          {
            std::lock_guard<decltype(lock)> guard(lock);
