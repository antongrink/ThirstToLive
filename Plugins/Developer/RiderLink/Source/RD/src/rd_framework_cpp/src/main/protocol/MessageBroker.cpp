...
std::shared_ptr<spdlog::logger> MessageBroker::logger =
    spdlog::stderr_color_mt<spdlog::synchronous_factory>(
        "logger", spdlog::color_mode::automatic);
...
void MessageBroker::invoke(const IRdReactive *that, Buffer msg,
                           bool sync) const {
  if (sync) {
    ...
    auto action = [this, that, message = std::move(msg)]() mutable {
      ...
      if (exists_id) {
        ...
        logger->trace("Disappeared Handler for Reactive entities with id: {}",
                      to_string(that->get_id()));
      }
    };
    std::function<void()> function =
        util::make_shared_function(std::move(action));
    that->get_wire_scheduler()->queue(std::move(function));
  }
}

MessageBroker::MessageBroker(IScheduler *defaultScheduler)
...
void MessageBroker::dispatch(RdId id, Buffer message) const {
  ...
  { // synchronized recursively
    ...
    if (s == nullptr) {
      ...
      auto action = [this, it, id]() mutable {
        ...
        optional<Buffer> message;
        {
          ...
          if (!current.default_scheduler_messages.empty()) {
            message = make_optional<Buffer>(
                std::move(current.default_scheduler_messages.front()));
        ...
        if (subscription != nullptr) {
          if (message) {
            invoke(subscription, *std::move(message),
                   subscription->get_wire_scheduler() == default_scheduler);
          }
        } else {
        ...
        if (current.default_scheduler_messages.empty()) {
          ...
          for (auto &it : t.custom_scheduler_messages) {
            RD_ASSERT_MSG(subscription->get_wire_scheduler() !=
                              default_scheduler,
      ...
      if (s->get_wire_scheduler() == default_scheduler ||
          s->get_wire_scheduler()->out_of_order_execution) {
...
void MessageBroker::advise_on(Lifetime lifetime,
                              IRdReactive const *entity) const {
  RD_ASSERT_MSG(
      !entity->get_id().isNull(),
      ("id is null for entities: " + std::string(typeid(*entity).name())))
