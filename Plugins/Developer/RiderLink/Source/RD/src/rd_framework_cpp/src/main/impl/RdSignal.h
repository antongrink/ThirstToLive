...
class RdSignal final : public RdReactiveBase,
                       public ISignal<T>,
...
public:
  ...
  void fire(T const &value) const override {
    // Disabling checking for bounding, to fix async signal being called unbound
    // on another thread. Copying hack from rd-net
  ...
  IScheduler *get_wire_scheduler() const override { return wire_scheduler; }

  friend std::string to_string(RdSignal const &) { return ""; }
};
} // namespace rd
...
static_assert(std::is_move_constructible<rd::RdSignal<int>>::value,
