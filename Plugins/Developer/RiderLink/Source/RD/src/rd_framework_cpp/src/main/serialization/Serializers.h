...
private:
  ...
  static void real_write(SerializationCtx &ctx, Buffer &buffer,
                         IUnknownInstance const &value);

  static void real_write(SerializationCtx &ctx, Buffer &buffer,
                         IPolymorphicSerializable const &value);
  ...
  mutable rd::unordered_map<
      RdId, std::function<InternedAny(SerializationCtx &, Buffer &)>>
      readers;
...
public:
  ...
  template <typename T, typename = typename std::enable_if_t<
                            util::is_base_of_v<IPolymorphicSerializable, T>>>
  void registry() const;
  ...
  template <typename T>
  value_or_wrapper<T> readPolymorphicNullable(SerializationCtx &ctx,
                                              Buffer &buffer) const;
  ...
  template <typename T>
  value_or_wrapper<T> readPolymorphic(SerializationCtx &ctx,
                                      Buffer &buffer) const;

  template <typename T>
  void writePolymorphic(SerializationCtx &ctx, Buffer &stream,
                        const Wrapper<T> &value) const;
...
namespace rd {
template <typename T, typename> void Serializers::registry() const {
  std::string type_name = T::static_type_name();
  util::hash_t h = util::getPlatformIndependentHash(type_name);
  RdId id(h);

  RD_ASSERT_MSG(readers.count(id) == 0,
                "Can't register " + type_name + " with id: " + to_string(id));

  readers[id] = [](SerializationCtx &ctx,
                   Buffer &buffer) -> Wrapper<IPolymorphicSerializable> {
    return wrapper::make_wrapper<T>(T::read(ctx, buffer));
  };
}

template <typename T>
optional<InternedAny> Serializers::readAny(SerializationCtx &ctx,
                                           Buffer &buffer) const {
  ...
  if (readers.count(id) == 0) {
    return any::make_interned_any<T>(
        T::readUnknownInstance(ctx, buffer, id, size));
  }
...
template <typename T>
value_or_wrapper<T> Serializers::readPolymorphicNullable(SerializationCtx &ctx,
                                                         Buffer &buffer) const {
...
void Serializers::writePolymorphicNullable(SerializationCtx &ctx,
                                           Buffer &buffer,
                                           const T &value) const {
...
template <typename T>
value_or_wrapper<T> Serializers::readPolymorphic(SerializationCtx &ctx,
                                                 Buffer &buffer) const {
  return readPolymorphicNullable<T>(ctx, buffer);
}

template <typename T>
void Serializers::writePolymorphic(SerializationCtx &ctx, Buffer &stream,
                                   const Wrapper<T> &value) const {
