...
std::shared_ptr<spdlog::logger> ByteBufferAsyncProcessor::logger =
    spdlog::stderr_color_mt<spdlog::synchronous_factory>(
        "byteBufferLog", spdlog::color_mode::automatic);

ByteBufferAsyncProcessor::ByteBufferAsyncProcessor(
    std::string id,
    std::function<bool(Buffer::ByteArray const &, sequence_number_t)> processor)
...
bool ByteBufferAsyncProcessor::terminate0(time_t timeout,
                                          StateKind state_to_set,
                                          string_view action) {
  {
    std::lock_guard<decltype(lock)> guard(lock);
    if (state == StateKind::Initialized) {
      logger->debug("Can't {} \'{}\', because it hasn't been started yet",
    ...
    if (state >= state_to_set) {
      logger->debug("Trying to {} async processor \'{}' but it's in state {}",
                    std::string(action), id, to_string(state));
      return true;
    }
  ...
  if (status == std::future_status::timeout) {
    logger->error("Couldn't wait async thread during time: {}",
                  to_string(timeout));
    success = false;
  }
...
void ByteBufferAsyncProcessor::add_data(
    std::vector<Buffer::ByteArray> &&new_data) {
...
void ByteBufferAsyncProcessor::ThreadProc() {
  rd::util::set_thread_name(id.empty() ? "ByteBufferAsyncProcessor Thread"
                                       : id.c_str());
...
void ByteBufferAsyncProcessor::start() {
  {
    ...
    if (state != StateKind::Initialized) {
      logger->debug("Trying to START async processor {} but it's in state {}",
                    id, to_string(state));
      return;
    }
    ...
    async_future = std::async(std::launch::async,
                              &ByteBufferAsyncProcessor::ThreadProc, this);
  }
}
...
void ByteBufferAsyncProcessor::pause(const std::string &reason) {
  ...
  logger->debug("{} paused with reason={},state={}", id, reason,
                to_string(state));

  auto current_thread_id = std::this_thread::get_id();
  if (current_thread_id != async_thread_id) {
    logger->debug("{} paused from another thread : {}", id,
                  to_string(current_thread_id));
    std::unique_lock<decltype(processing_lock)> ul(processing_lock);
    processing_cv.wait(ul, [this]() -> bool { return !in_processing; });
...
void ByteBufferAsyncProcessor::acknowledge(sequence_number_t seqn) {
  ...
  if (seqn > acknowledged_seqn) {
    ...
    logger->error(
        "Acknowledge {} called, while next seqn MUST BE greater than {}", seqn,
        acknowledged_seqn);
  }
}
