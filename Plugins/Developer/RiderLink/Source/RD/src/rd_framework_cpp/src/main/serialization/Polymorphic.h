 ...
 */
template <typename T, typename R = void> class Polymorphic {
public:
  inline static T read(SerializationCtx &ctx, Buffer &buffer) {
    return T::read(ctx, buffer);
  }

  inline static void write(SerializationCtx &ctx, Buffer &buffer,
                           T const &value) {
...
class Polymorphic<C<T, A>, typename std::enable_if_t<
                               !util::is_base_of_v<RdReactiveBase, T> &&
...
template <> class Polymorphic<bool> {
public:
...
template <> class Polymorphic<wchar_t> {
public:
...
public:
  ...
  inline static void write(SerializationCtx & /*ctx*/, Buffer &buffer,
                           Wrapper<std::wstring> const &value) {
    buffer.write_wstring(*value);
  }
};

template <> class Polymorphic<DateTime> {
public:
...
template <> class Polymorphic<Void> {
public:
...
template <typename T>
class Polymorphic<
    T, typename std::enable_if_t<util::is_base_of_v<RdReactiveBase, T>>> {
public:
  inline static T read(SerializationCtx &ctx, Buffer &buffer) {
    return T::read(ctx, buffer);
  }
...
template <typename T> class Polymorphic<optional<T>> {
public:
  inline static optional<T> read(SerializationCtx &ctx, Buffer &buffer) {
    return buffer.read_nullable<T>(
        [&ctx, &buffer]() { return Polymorphic<T>::read(ctx, buffer); });
  }
...
template <typename T, typename A> class Polymorphic<Wrapper<T, A>> {
public:
  inline static void write(SerializationCtx &ctx, Buffer &buffer,
                           Wrapper<T, A> const &value) {
    value->write(ctx, buffer);
  }
};
} // namespace rd
