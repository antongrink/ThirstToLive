...
size_t Buffer::get_position() const { return offset; }

void Buffer::set_position(size_t value) { offset = value; }

void Buffer::check_available(size_t moreSize) const {
  if (offset + moreSize > size()) {
    throw std::out_of_range("Expected " + std::to_string(moreSize) +
                            " bytes in buffer, only" +
                            std::to_string(size() - offset) + "available");
  }
}
...
void Buffer::rewind() { set_position(0); }

Buffer::ByteArray Buffer::getArray() const & { return data_; }
...
Buffer::word_t const *Buffer::data() const { return data_.data(); }

Buffer::word_t *Buffer::data() { return data_.data(); }

Buffer::word_t const *Buffer::current_pointer() const {
  return data() + offset;
}

Buffer::word_t *Buffer::current_pointer() { return data() + offset; }
...
template <int> std::wstring read_wstring_spec(Buffer &buffer) {
  auto v = buffer.read_array<std::vector, uint16_t>();
  return std::wstring(v.begin(), v.end());
}

template <> std::wstring read_wstring_spec<2>(Buffer &buffer) {
  const int32_t len = buffer.read_integral<int32_t>();
  RD_ASSERT_MSG(len >= 0,
                "read null string(length =" + std::to_string(len) + ")");
  std::wstring result;
  result.resize(len);
  buffer.read(reinterpret_cast<Buffer::word_t *>(&result[0]),
              sizeof(wchar_t) * len);
  return result;
}
...
template <int> void write_wstring_spec(Buffer &buffer, wstring_view value) {
  const std::vector<uint16_t> v(value.begin(), value.end());
  buffer.write_array<std::vector, uint16_t>(v);
}

template <> void write_wstring_spec<2>(Buffer &buffer, wstring_view value) {
  buffer.write_integral<int32_t>(static_cast<int32_t>(value.size()));
  buffer.write(reinterpret_cast<Buffer::word_t const *>(value.data()),
...
DateTime Buffer::read_date_time() {
  ...
  time_t t = static_cast<time_t>(time_in_ticks / TICKS_PER_MILLISECOND -
                                 TICKS_AT_EPOCH / TICKS_PER_MILLISECOND);
  return DateTime{t};
}
...
bool Buffer::read_bool() {
  ...
  RD_ASSERT_MSG(res == 0 || res == 1,
                "get byte:" + std::to_string(res) + " instead of 0 or 1");
  return res == 1;
}

void Buffer::write_bool(bool value) { write_integral<word_t>(value ? 1 : 0); }

wchar_t Buffer::read_char() {
  return static_cast<wchar_t>(read_integral<uint16_t>());
}

void Buffer::write_char(wchar_t value) { write_integral<uint16_t>(value); }
...
Buffer::ByteArray &Buffer::get_data() { return data_; }
} // namespace rd
