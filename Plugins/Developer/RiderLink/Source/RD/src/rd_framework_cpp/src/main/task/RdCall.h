...
namespace rd {
/**
 * \brief Represents an API provided by the remote process which can be invoked
 ...
 */
template <typename TReq, typename TRes, typename ReqSer = Polymorphic<TReq>,
          typename ResSer = Polymorphic<TRes>>
...
public:
  ...
  static RdCall<TReq, TRes, ReqSer, ResSer> read(SerializationCtx & /*ctx*/,
                                                 Buffer &buffer) {
  ...
  /**
   * \brief Invokes the API with the parameters given as [request] and waits for
   * the result.
   *
   * \param request value to deliver
   * \return result of remote invoking
   */
  WiredRdTask<TRes, ResSer>
  sync(TReq const &request, std::chrono::milliseconds timeout = 200ms) const {
    auto task =
        start_internal(request, true, &SynchronousScheduler::Instance());
    ...
    spdlog::debug("Time elapsed: {}, has_value={}",
                  to_string(std::chrono::system_clock::now() - time_at_start),
  ...
  /**
   * \brief Asynchronously invokes the API with the parameters given as
   ...
   */
  WiredRdTask<TRes, ResSer>
  start(TReq const &request, IScheduler *responseScheduler = nullptr) const {
    return start_internal(request, false,
...
private:
  WiredRdTask<TRes, ResSer> start_internal(TReq const &request, bool sync,
    ...
    if (sync) {
      if (sync_task_id.has_value()) {
        throw std::invalid_argument(
            "Already exists sync task for call " + to_string(location) +
            ", taskId = " + rd::to_string(*sync_task_id));
      }
      sync_task_id = task_id;
    }

    get_wire()->send(rdid, [&](Buffer &buffer) {
      spdlog::get("logSend")->trace("call {}::{} send {} request {} : {}",
                                    to_string(location), to_string(rdid),
                                    (sync ? "SYNC" : "ASYNC"),
...
public:
  ...
  friend std::string to_string(RdCall const & /*value*/) { return "RdCall"; }
};
} // namespace rd
