...
namespace rd {
/**
 * \brief An API that is exposed to the remote process and can be invoked over
 * the protocol.
 *
...
class RdEndpoint : public virtual RdReactiveBase, public ISerializable {
  ...
  mutable tsl::ordered_map<RdId, RdTask<TRes, ResSer>, rd::hash<RdId>>
      awaiting_tasks; // TO-DO get rid of it
public:
  // region ctor/dtor

  RdEndpoint() = default;

  explicit RdEndpoint(handler_t handler) { set(std::move(handler)); }
  ...
  static RdEndpoint<TReq, TRes, ReqSer, ResSer> read(SerializationCtx & /*ctx*/,
                                                     Buffer &buffer) {
  ...
  void set(std::function<WTRes(TReq const &)> functor) const {
    local_handler = [handler = std::move(functor)](
                        Lifetime _, TReq const &req) -> RdTask<TRes, ResSer> {
  ...
  void on_wire_received(Buffer buffer) const override {
    ...
    spdlog::get("logReceived")
        ->trace("endpoint {}::{} request = {}", to_string(location),
                to_string(rdid), to_string(value));
    if (!local_handler) {
    ...
    task.advise(
        *bind_lifetime,
        [this, task_id, &task](RdTaskResult<TRes, ResSer> const &task_result) {
          spdlog::get("logSend")->trace("endpoint {}::{} response = {}",
                                        to_string(location), to_string(rdid),
                                        to_string(*task.result));
