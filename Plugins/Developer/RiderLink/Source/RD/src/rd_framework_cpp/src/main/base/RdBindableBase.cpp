...
bool RdBindableBase::is_bound() const { return parent != nullptr; }

void RdBindableBase::bind(Lifetime lf, IRdDynamic const *parent,
                          string_view name) const {
  RD_ASSERT_MSG(!is_bound(), ("Trying to bind already bound this to " +
...
void RdBindableBase::identify(const Identities &identities,
                              RdId const &id) const {
...
const RName &RdBindableBase::get_location() const { return location; }

RdId RdBindableBase::get_id() const { return rdid; }

void RdBindableBase::set_id(RdId id) const { rdid = id; }
