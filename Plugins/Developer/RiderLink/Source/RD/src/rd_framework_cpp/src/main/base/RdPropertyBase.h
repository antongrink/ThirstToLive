...
public:
  ...
  bool is_default_value_changed() const { return default_value_changed; }
    ...
    advise(lifetime, [this](T const &v) {
      ...
      get_wire()->send(rdid, [this, &v](Buffer &buffer) {
        ...
        spdlog::get("logSend")->trace(
            "SEND property {} + {}:: ver = {}, value = {}", to_string(location),
            to_string(rdid), std::to_string(master_version), to_string(v));
      });
    });
  ...
  void on_wire_received(Buffer buffer) const override {
    ...
    spdlog::get("logSend")->trace(
        "RECV property {} {}:: oldver={}, ver={}, value = {}{}",
        to_string(location), to_string(rdid), master_version, version,
        to_string(v), (rejected ? ">> REJECTED" : ""));
    if (rejected) {
      return;
    }
