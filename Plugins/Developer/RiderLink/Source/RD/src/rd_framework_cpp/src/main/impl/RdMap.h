 ...
 */
template <typename K, typename V, typename KS = Polymorphic<K>,
          typename VS = Polymorphic<V>, typename KA = std::allocator<K>,
          typename VA = std::allocator<V>>
class RdMap final : public RdReactiveBase,
                    public ViewableMap<K, V, KA, VA>,
...
private:
  ...
  mutable ordered_map<K const *, int64_t, wrapper::TransparentHash<K>,
                      wrapper::TransparentKeyEqual<K>>
      pendingForAck;

  std::string logmsg(Op op, int64_t version, K const *key,
                     V const *value = nullptr) const {
    return "map " + to_string(location) + " " + to_string(rdid) +
           ":: " + to_string(op) + ":: key = " + to_string(*key) +
           ((version > 0) ? " :: version = " + std::to_string(version) : "") +
           " :: value = " + (value ? to_string(*value) : "");
  }

  std::string logmsg(Op op, int64_t version, K const *key,
                     optional<WV> const &value) const {
...
public:
  ...
  void init(Lifetime lifetime) const override {
    ...
    local_change([this, lifetime]() {
      advise(lifetime, [this, lifetime](Event e) {
        ...
        get_wire()->send(rdid, [this, e](Buffer &buffer) {
          ...
          buffer.write_integral<int32_t>(static_cast<int32_t>(op) |
                                         versionedFlag);
          ...
          spdlog::get("logSend")->trace(
              "SEND{}", logmsg(op, next_version - 1, e.get_key(), new_value));
        });
      });
    });
    ...
    if (!optimize_nested)
      this->view(lifetime,
                 [this](Lifetime lf, std::pair<K const *, V const *> entry) {
                   bindPolymorphic(entry.second, lf, this,
  ...
  void on_wire_received(Buffer buffer) const override {
    ...
    if (op == Op::ACK) {
      std::string errmsg;
      if (!msg_versioned) {
        errmsg = "Received " + to_string(Op::ACK) +
                 " while msg hasn't versioned flag set";
      } else if (!is_master) {
        ...
        if (pendingForAck.count(key) > 0) {
          ...
          if (pendingVersion < version) {
            errmsg = "Pending version " + std::to_string(pendingVersion) +
                     " < " + to_string(Op::ACK) + " version `" +
                     std::to_string(version);
          } else {
            // side effect
            if (pendingVersion == version) {
              pendingForAck.erase(
                  key); // else we don't need to remove, silently drop
            }
            // return good result
          }
        } else {
      ...
      if (errmsg.empty()) {
        spdlog::get("logReceived")
            ->trace(logmsg(Op::ACK, version, &(wrapper::get<K>(key))));
      } else {
        spdlog::get("logReceived")
            ->error(logmsg(Op::ACK, version, &(wrapper::get<K>(key))) + " >> " +
                    errmsg);
      }
    } else {
      Buffer serialized_key;
      KS::write(this->get_serialization_context(), serialized_key,
                wrapper::get<K>(key));
      ...
      if (msg_versioned || !is_master || pendingForAck.count(key) == 0) {
        spdlog::get("logReceived")
            ->trace("RECV{}",
                    logmsg(op, version, &(wrapper::get<K>(key)), value));
        if (value.has_value()) {
      ...
      if (msg_versioned) {
        auto writer = util::make_shared_function(
            [version, serialized_key = std::move(serialized_key)](
              ...
              // KS::write(this->get_serialization_context(), innerBuffer,
              // wrapper::get<K>(key));
        ...
        if (is_master) {
          spdlog::get("logReceived")
              ->error("Both ends are masters: {}", to_string(location));
        }
      }
    }
  }
  ...
  void advise(Lifetime lifetime,
              std::function<void(Event const &)> handler) const override {
    if (is_bound()) {
      assert_threading();
    }
  ...
  V const *set(WK key, WV value) const override {
    return local_change(
        [&]() mutable { return map::set(std::move(key), std::move(value)); });
  }
...
static_assert(std::is_move_constructible<rd::RdMap<int, int>>::value,
              "Is move constructible RdMap<int, int>");
static_assert(std::is_move_assignable<rd::RdMap<int, int>>::value,
              "Is move constructible RdMap<int, int>");
static_assert(std::is_default_constructible<rd::RdMap<int, int>>::value,
              "Is default constructible RdMap<int, int>");
