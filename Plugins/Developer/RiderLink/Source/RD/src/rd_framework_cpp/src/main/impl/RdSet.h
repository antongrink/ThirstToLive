...
class RdSet final : public RdReactiveBase,
                    public ViewableSet<T, A>,
...
public:
    ...
    local_change([this, lifetime] {
      advise(lifetime, [this](AddRemove kind, T const &v) {
        ...
        get_wire()->send(rdid, [this, kind, &v](Buffer &buffer) {
          ...
          spdlog::get("logSend")->trace("SENDset {} {}:: {}:: {}",
                                        to_string(location), to_string(rdid),
                                        to_string(kind), to_string(v));
        });
      });
    });
  ...
  bool add(WT value) const override {
    return local_change([this, value = std::move(value)]() mutable {
      return set::add(std::move(value));
    });
  }
  ...
  bool addAll(std::vector<WT> elements) const override {
    return local_change([this, elements = std::move(elements)]() mutable {
...
static_assert(std::is_move_constructible<rd::RdSet<int>>::value,
              "Is move constructible RdSet<int>");
