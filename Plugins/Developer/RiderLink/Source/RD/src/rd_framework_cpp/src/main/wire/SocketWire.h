...
public:
  ...
  protected:
    ...
    mutable ByteBufferAsyncProcessor async_send_buffer{
        id + "-AsyncSendProcessor",
    ...
    mutable decltype(receiver_buffer)::iterator lo = receiver_buffer.begin(),
                                                hi = receiver_buffer.begin();

    static constexpr int32_t ACK_MESSAGE_LENGTH = -1;
    static constexpr int32_t PING_MESSAGE_LENGTH = -2;
    static constexpr int32_t PACKAGE_HEADER_LENGTH =
        sizeof(ACK_MESSAGE_LENGTH) + sizeof(sequence_number_t);
    mutable Buffer ack_buffer{PACKAGE_HEADER_LENGTH};

    /**
     * \brief Timestamp of this wire which increases at intervals of
     * [heartBeatInterval].
     */
    ...
    /**
     * \brief The latest received counterpart's acknowledge of this wire's
     * [currentTimeStamp].
     */
    ...
    mutable PkgInputStream receive_pkg{
        [this]() -> int32_t { return this->read_package(); }};

    mutable Buffer message{CHUNK_SIZE};

    bool read_from_socket(Buffer::word_t *res, int32_t msglen) const;

    template <typename T> bool read_integral_from_socket(T &x) const {
      return read_from_socket(reinterpret_cast<Buffer::word_t *>(&x),
                              sizeof(T));
    }

    bool read_data_from_socket(Buffer::word_t *data, size_t len) const {
      return read_from_socket(reinterpret_cast<Buffer::word_t *>(data),
  ...
  public:
    ...
    void send(RdId const &rd_id,
              std::function<void(Buffer &buffer)> writer) const override;

    static bool connection_established(int32_t timestamp,
                                       int32_t acknowledged_timestamp);
  ...
  public:
    ...
    Client(Lifetime parentLifetime, IScheduler *scheduler, uint16_t port = 0,
           const std::string &id = "ClientSocket");
  ...
  public:
    ...
    Server(Lifetime lifetime, IScheduler *scheduler, uint16_t port = 0,
           const std::string &id = "ServerSocket");
