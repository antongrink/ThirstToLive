...
namespace rd {
template <typename, typename> class WiredRdTask;
...
public:
  template <typename, typename> friend class ::rd::WiredRdTask;

  WiredRdTaskImpl(Lifetime lifetime, RdReactiveBase const &cutpoint, RdId rdid,
                  IScheduler *scheduler, Property<RdTaskResult<T, S>> *result)
      : lifetime(lifetime), cutpoint(&cutpoint), scheduler(scheduler),
        result(result) {
  ...
  void on_wire_received(Buffer buffer) const override {
    auto read_result =
        RdTaskResult<T, S>::read(cutpoint->get_serialization_context(), buffer);
    spdlog::get("logReceived")
        ->trace("call {} {} received response {} : {}",
                to_string(cutpoint->get_location()), to_string(rdid),
                to_string(rdid), to_string(read_result));
    scheduler->queue([&, result = std::move(read_result)]() mutable {
      if (this->result->has_value()) {
        spdlog::get("logReceived")
            ->trace("call {} {} response was dropped, task result is: {}",
