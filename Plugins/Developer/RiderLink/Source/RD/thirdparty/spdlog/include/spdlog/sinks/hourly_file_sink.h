...
struct hourly_filename_calculator {
  ...
  static filename_t calc_filename(const filename_t &filename,
    ...
    return fmt_lib::format(
        SPDLOG_FILENAME_T("{}_{:04d}-{:02d}-{:02d}_{:02d}{}"), basename,
...
public:
  ...
  hourly_file_sink(filename_t base_filename, bool truncate = false,
                   uint16_t max_files = 0,
                   const file_event_handlers &event_handlers = {})
      : base_filename_(std::move(base_filename)), file_helper_{event_handlers},
  ...
  void delete_old_() {
    ...
    if (filenames_q_.full()) {
      ...
      if (!ok) {
        ...
        SPDLOG_THROW(spdlog_ex("Failed removing hourly file " +
