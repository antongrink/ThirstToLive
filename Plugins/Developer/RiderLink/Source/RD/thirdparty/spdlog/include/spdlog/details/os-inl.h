...
SPDLOG_INLINE bool fopen_s(FILE **fp, const filename_t &filename,
  ...
  *fp = ::_fsopen((filename.c_str()), mode.c_str(), _SH_DENYNO);
  ...
  const int fd =
      ::open((filename.c_str()), O_CREAT | O_WRONLY | O_CLOEXEC | mode_flag,
             mode_t(0644));
  if (fd == -1) {
    return true;
  }
...
SPDLOG_INLINE int utc_minutes_offset(const std::tm &tm) {
  ...
  struct helper {
    static long int
    calculate_gmt_offset(const std::tm &localtm = details::os::localtime(),
...
#define SYS_gettid __NR_gettid
  ...
  int retval = pthread_getthrds_np(&pt, PTHRDSINFO_QUERY_TID, &buf, sizeof(buf),
                                   NULL, &reg_size);
...
SPDLOG_INLINE void wstr_to_utf8buf(wstring_view_t wstr, memory_buf_t &target) {
  ...
  throw_spdlog_ex(fmt_lib::format("WideCharToMultiByte failed. Last error: {}",
                                  ::GetLastError()));
}
...
SPDLOG_INLINE void utf8_to_wstrbuf(string_view_t str, wmemory_buf_t &target) {
  ...
  if (result_size > 0) {
    ...
    result_size =
        ::MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, str.data(),
                              str_size, target.data(), result_size);
  ...
  throw_spdlog_ex(fmt_lib::format("MultiByteToWideChar failed. Last error: {}",
                                  ::GetLastError()));
}
#endif // (defined(SPDLOG_WCHAR_TO_UTF8_SUPPORT) ||
       // defined(SPDLOG_WCHAR_FILENAMES)) && defined(_WIN32)
