...
protected:
  ...
  void sink_it_(const details::log_msg &msg) override {
    ...
    if (skip_counter_ > 0) {
      char buf[64];
      auto msg_size =
          ::snprintf(buf, sizeof(buf), "Skipped %u duplicate messages..",
                     static_cast<unsigned>(skip_counter_));
      if (msg_size > 0 && static_cast<size_t>(msg_size) < sizeof(buf)) {
        details::log_msg skipped_msg{
            msg.logger_name, level::info,
    ...
    skip_counter_ = 0;
    last_msg_payload_.assign(msg.payload.data(),
  ...
  bool filter_(const details::log_msg &msg) {
    ...
    return (filter_duration > max_skip_duration_) ||
           (msg.payload != last_msg_payload_);
  }
};
