...
public:
  scoped_padder(size_t wrapped_size, const padding_info &padinfo,
    ...
    remaining_pad_ =
        static_cast<long>(padinfo.width_) - static_cast<long>(wrapped_size);
    if (remaining_pad_ <= 0) {
      return;
    }
  ...
  template <typename T> static unsigned int count_digits(T n) {
...
private:
  void pad_it(long count) {
    fmt_helper::append_string_view(
        string_view_t(spaces_.data(), static_cast<size_t>(count)), dest_);
  }
...
struct null_scoped_padder {
  null_scoped_padder(size_t /*wrapped_size*/, const padding_info & /*padinfo*/,
                     memory_buf_t & /*dest*/) {}

  template <typename T> static unsigned int count_digits(T /* number */) {
    return 0;
  }
};
...
public:
  ...
  void format(const details::log_msg &, const std::tm &tm_time,
    ...
    fmt_helper::append_string_view(days[static_cast<size_t>(tm_time.tm_wday)],
                                   dest);
    dest.push_back(' ');
    fmt_helper::append_string_view(months[static_cast<size_t>(tm_time.tm_mon)],
                                   dest);
    dest.push_back(' ');
...
#pragma warning(disable : 4127) // consider using 'if constexpr' instead
  ...
  static const char *basename(const char *filename) {
    ...
    if (sizeof(os::folder_seps) == 2) {
      ...
      const std::reverse_iterator<const char *> begin(filename +
                                                      std::strlen(filename));
      const std::reverse_iterator<const char *> end(filename);

      const auto it =
          std::find_first_of(begin, end, std::begin(os::folder_seps),
...
public:
  ...
  void format(const details::log_msg &msg, const std::tm &,
              memory_buf_t &dest) override {
    auto delta =
        (std::max)(msg.time - last_message_time_, log_clock::duration::zero());
...
SPDLOG_INLINE pattern_formatter::pattern_formatter(
    ...
    : pattern_(std::move(pattern)), eol_(std::move(eol)),
      pattern_time_type_(time_type), need_localtime_(false), last_log_secs_(0),
...
pattern_formatter::handle_flag_(char flag, details::padding_info padding) {
  ...
  case ('O'): // elapsed time since last log message in seconds
    formatters_.push_back(
        details::make_unique<
            details::elapsed_formatter<Padder, std::chrono::seconds>>(padding));
    break;
  ...
  default: // Unknown flag appears as is
    ...
    // fix issue #1617 (prev char was '!' and should have been treated as
    // funcname flag instead of truncating flag) spdlog::set_pattern("[%10!]
    // %v") => "[      main] some message" spdlog::set_pattern("[%3!!] %v") =>
    // "[mai] some message"
    else {
