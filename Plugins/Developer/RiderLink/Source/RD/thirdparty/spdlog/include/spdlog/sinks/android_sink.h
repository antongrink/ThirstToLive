...
/*
 * Android sink
 * (logging using __android_log_write or __android_log_buf_write depending on
 * the specified BufferID)
 */
...
private:
  // There might be liblog versions used, that do not support
  // __android_log_buf_write. So we only compile and link against
  // __android_log_buf_write, if user explicitely provides a non-default log
  // buffer. Otherwise, when using the default log buffer, always log via
  // __android_log_write.
  template <int ID = BufferID>
  typename std::enable_if<ID == static_cast<int>(log_id::LOG_ID_MAIN),
                          int>::type
