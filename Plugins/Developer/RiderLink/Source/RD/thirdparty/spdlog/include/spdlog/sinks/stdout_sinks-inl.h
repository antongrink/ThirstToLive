...
// #1675) so instead we use ::FileWrite
...
SPDLOG_INLINE stdout_sink_base<ConsoleMutex>::stdout_sink_base(FILE *file)
  ...
  // throw only if non stdout/stderr target is requested (probably regular file
  // and not console).
...
template <typename ConsoleMutex>
SPDLOG_INLINE void
stdout_sink_base<ConsoleMutex>::log(const details::log_msg &msg) {
#ifdef _WIN32
...
template <typename ConsoleMutex>
SPDLOG_INLINE void
stdout_sink_base<ConsoleMutex>::set_pattern(const std::string &pattern) {
  std::lock_guard<mutex_t> lock(mutex_);
  formatter_ =
