...
#define SPDLOG_LOGGER_CATCH(location)                                          \
    ...
    if (location.filename) {                                                   \
      err_handler_(fmt_lib::format("{} [{}({})]", ex.what(),                   \
...
public:
  ...
  void log(source_loc loc, level::level_enum lvl, format_string_t<Args...> fmt,
           Args &&...args) {
  ...
  template <typename T> void warn(const T &msg) { log(level::warn, msg); }

  template <typename T> void error(const T &msg) { log(level::err, msg); }
...
protected:
  ...
  void log_(source_loc loc, level::level_enum lvl, string_view_t fmt,
    ...
    SPDLOG_TRY {
      ...
      fmt_lib::vformat_to(
          std::back_inserter(buf), fmt,
          fmt_lib::make_format_args(std::forward<Args>(args)...));
#else
      // seems that fmt::detail::vformat_to(buf, ...) is ~20ns faster than
      // fmt::vformat_to(std::back_inserter(buf),..)
      fmt::detail::vformat_to(
          buf, fmt, fmt::make_format_args(std::forward<Args>(args)...));
#endif
  ...
  void log_(source_loc loc, level::level_enum lvl, wstring_view_t fmt,
    ...
    SPDLOG_TRY {
      ...
      fmt::detail::vformat_to(wbuf, fmt,
                              fmt::make_format_args<fmt::wformat_context>(
                                  std::forward<Args>(args)...));
#endif

      memory_buf_t buf;
      details::os::wstr_to_utf8buf(wstring_view_t(wbuf.data(), wbuf.size()),
                                   buf);
      details::log_msg log_msg(loc, name_, lvl,
                               string_view_t(buf.data(), buf.size()));
      log_it_(log_msg, log_enabled, traceback_enabled);
  ...
  template <class T,
            typename std::enable_if<
                std::is_convertible<const T &, spdlog::wstring_view_t>::value,
                int>::type = 0>
